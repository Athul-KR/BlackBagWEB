...
$(document).ready(function() {
  ...
  function importDoctors() {
    ...
    if(iserrorRecords !='1'){
      ...
      $("#submit-btn").addClass('disabled');
        $.ajax({
            url: '<?php echo e(url("/doctors/import/store")); ?>',
            type: "post",
            data: {
              'importKey' : importKey,
              '_token': $('meta[name="csrf-token"]').attr('content')
            ...
            success: function(data) {
              if (data.success == 1) {
                ...
                window.location.href = "<?php echo e(url('doctors/list/pending')); ?>" ;
              } else {
                swal({
                  icon: 'error',
                  text: data.message,
  ...
  function removeDocument(importKey) {
        ...
        $.ajax({
            url: '<?php echo e(url("/doctors/import/delete")); ?>',
            type: "post",
            data: {
              'importKey' : importKey,
              '_token'    : $('meta[name="csrf-token"]').attr('content')
   ...
   function viewDetails(importKey) {
    ...
    $.ajax({
        url: '<?php echo e(url("/doctors/previewdetails")); ?>',
        type: "post",
        data: {
         'importKey' : importKey,
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\Xampp\htdocs\blackbag\resources\views/doctors/excelpreview.blade.php ENDPATH**/ ?>