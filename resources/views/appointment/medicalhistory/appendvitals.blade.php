            ...
            <div class="accordion-item">
                <h2 class="accordion-header" id="{{$headingId}}">
                    <button class="accordion-button collapsed accrclp_{{$history['type']}}" type="button"
                        ...
                        <div class="row align-items-center collapse-header w-100 g-2">
                            <div @if($viewType=='video' ) class="col-7 col-xl-3 order-0 order-xl-0" @elseif($history['type']=='blood_pressure' ) class="col-8 col-lg-2 order-0 order-lg-0" @else class="col-8 col-lg-2 order-0 order-lg-0" @endif>
                                <h5 class="fw-medium">{{$history['title']}}</h5>
                            </div>
                            @if($history['value1'] != null && $history['type'] == 'blood_pressure')
                            <div @if($viewType=='video' ) class="col-xl-8 order-2 order-xl-1 video accrclpopen_{{$history['type']}}" @else class="col-12 col-lg-10 order-2 order-lg-1 accrclpopen_{{$history['type']}}" @endif>
                                <div @if($viewType=='video' ) class="row text-end pe-3" @else class="row text-end pe-lg-3 mt-lg-0 mt-3" @endif>
                                    <div class="col-8">
                                        <small>{{$history['date']}}</small>
                                        <p class="l-primary mt-1 mb-0">{{$history['value'].'/'.$history['value1']}} {{$history['unit']}} (sys/dia)</p>
                                    </div>
                                    <!-- <div class="col-4">
                            ...
                            @elseif($history['value1'] != null && $history['type'] == 'blood_glucose')
                            <div @if($viewType=='video' ) class="col-xl-8 order-2 order-xl-1 video accrclpopen_{{$history['type']}}" @else class="col-12 col-lg-10 order-2 order-lg-1 accrclpopen_{{$history['type']}}" @endif>
                                <div @if($viewType=='video' ) class="row text-end pe-3 justify-content-end" @else class="row text-end pe-lg-3 mt-lg-0 mt-3 justify-content-end" @endif>
                                    <div class="col-4">
                                        <small>{{$history['date']}}</small>
                                    ...
                                    <div class="col-4">
                                        <small class="mb-1">{{$history['date']}}</small>
                                        <p class="l-danger fw-medium mt-1 mb-0">@if($history['value1'] !='') {{$history['value1']}} mg/dL (hdl) @else -- @endif</p>
                            ...
                            @endif
                            <div class="@if($viewType == 'video' ) col-5 col-xl-3 order-1 order-xl-2 @elseif($history['type'] == 'blood_pressure') col-4 col-lg-2 @else col-4 col-lg-2 @endif addcls_{{$history['type']}} order-1 order-lg-2" style="display:none">
                                @if($history['type'] != 'bmi')
                ...
                <div id="{{$collapseId}}" class="accordion-collapse collapse"
                    ...
                    <div class="accordion-body">
                        <div @if($viewType=='video' ) class="row" @else class="row mb-3" @endif>
                            <div class="col-6">
                                <ul class="nav nav-pills gap-2 d-flex flex-nowrap mb-xl-0 mb-3" id="pills-tab" role="tablist">
