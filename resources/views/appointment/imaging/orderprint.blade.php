                ...
                <tr style="border:1px solid transparent !important;">
                    <td colspan="3" style="padding:0px;border:0">
                        <table class="table mb-0" style="border:0;width:100%"> 
                            ...
                            <tr style="border:1px solid transparent !important;">
                                <td colspan="3" style="padding: 0px;padding-bottom:10px;border-bottom:1px solid #F3F3F3">
                                    <table style="border-collapse: collapse;width:100%">
                                        <tr style="border:1px solid transparent !important;"> 
                                            ...
                                            <td colspan="1" style="vertical-align: top; padding-right: 10px;border-bottom:0;width:30%">
                                                <p style="font-size: 14px;font-feature-settings: 'lnum' 1;border-bottom:0;color: #181818"><i class="fas fa-envelope" style="margin-right: 5px;"></i> {{$clinicDetails['email']}}</p>
                                                <p style="font-size: 14px;font-feature-settings: 'lnum' 1;border-bottom:0;color: #181818"><i class="fas fa-phone" style="margin-right: 5px;"></i>@if(isset($countryCodedetails['country_code'])) {{ $countryCodedetails['country_code'] }} @endif  <?php echo $corefunctions->formatPhone($clinicDetails['phone_number']) ?></p>
                            ...
                            <tr style="border:1px solid transparent !important;">
                                <td style="padding:15px 0px;border-bottom:0;vertical-align:top">
                                    <table style="border-collapse: collapse;width:100%">
                                        <tr style="border:1px solid transparent !important;">
                                            <td width="10%" style="vertical-align: top; padding-right: 10px;border-bottom:0;padding-top: 0">
                                                <img src="{{$patientDetails['logo_path']}}" style="width: 40px; height: 40px; object-fit: cover;border-radius: 50%;" alt="Patient">
                                            </td>
                                            <td width="90%" style="vertical-align: middle;border-bottom:0;padding-top: 0">
                                                <table style="border-collapse: collapse;width:100%">
                                                    <tr style="border:1px solid transparent !important;">
                                                        <td style="font-weight: 500; font-size: 14px;border-bottom:0">
                                                           <h6 class="primary fw-medium m-0">{{$patientDetails['first_name']}} {{$patientDetails['last_name']}}</h6>
                                                        </td>
                                                    </tr>
                                                    <tr style="border:1px solid transparent !important;">
                                                        <td style="font-size: 14px;font-feature-settings: 'lnum' 1;border-bottom:0;line-height: 1.425rem">
                                                            <p class="m-0" style="font-size: 14px;font-feature-settings: 'lnum' 1;border-bottom:0"> {{$patientDetails['age']}} | 
                                ...
                                <td style="padding:15px;border-bottom: 0px">
                                    ...
                                    <p class="m-0" style="font-size: 14px;font-feature-settings: 'lnum' 1;border-bottom:0">{{ $corefunctions -> showClinicanNameUser($userDetails,'0')}}</p>
                                ...
                                <td style="padding:15px;border-bottom: 0px;vertical-align:top">
                                    <h6 class="primary fw-medium m-0">Date of Order </h6>
                                    <p class="m-0" style="font-size: 14px;font-feature-settings: 'lnum' 1;border-bottom:0">@if(!empty($orderLists)) <?php echo $corefunctions->timezoneChange($orderLists['test_date'],"m/d/Y") ?> @else --  @endif</p>
                                </td>
                            </tr>
                            <tr style="border:1px solid transparent !important;">
                                <td colspan="3" align="center" style="border-top: 1px solid #F3F3F3;border-bottom:1px solid #F3F3F3;padding:15px 10px">
                                    <h5 class="primary fwt-bold mb-0">Radiology Order Form</h5>
                                </td>
                            </tr>
                            @foreach ($finalTestArray as $labTestId => $items) 
                            <tr style="border:1px solid transparent !important;">
                                <td colspan="3" style="padding:0px;padding-top:10px;border-bottom:0"> 
                                    <table style="width: 100%"> 
                                        ...
                                        <tr style="border:1px solid transparent !important;">
                                            <td style="padding-top: 10px;">
                                                <table style="width: 100%;">
                                                    <tr style="border:1px solid transparent !important;">
                                                        <td style="border: 1px solid #F3F3F3;padding;0px">
                                                            <table style="width: 100%;">
                                                                <tr style="border:1px solid transparent !important;">
                                                                    <td  @if(isset($item['sub_lab_test_id']) && isset($categoryDetails[$item['sub_lab_test_id']])) colspan="2" @else colspan="3" @endif style="padding: 10px;border: 0">
                                                                        @if(isset($item['sub_lab_test_id']) && isset($categoryDetails[$item['sub_lab_test_id']]))
                                                                        <p class="primary fw-medium">{{$categoryDetails[$item['sub_lab_test_id']]['name']}} @if(isset($item['options']) && !empty($item['options'])) ({{ implode(', ', $item['options']) }}) @endif  @if($item['is_contrast'] !='')({{ $item['is_contrast'] ? 'With Contrast' : 'Without Contrast' }}) @endif</p>
                                                                        @endif
                                                                        @if(isset($item['description']) && $item['description'] !='')
                                                                        <span class="gray fw-light" style="font-style:italic">{!! nl2br(e($item['description'])) !!}</span>
                                                                        @endif
                                                                    </td>
                                                                    @if(isset($item['sub_lab_test_id']) && isset($categoryDetails[$item['sub_lab_test_id']]) && isset($imagingcptCodes[$item['sub_lab_test_id']]) )
                                                                    <td colspan="1" style="padding: 10px;border:0;text-align:end">
                                                                        <p class="fw-medium primary mb-0"><span class="gray fw-light">CPT: </span>@if(isset($cptCodes[$imagingcptCodes[$item['sub_lab_test_id']]['cpt_code_id']])) {{$cptCodes[$imagingcptCodes[$item['sub_lab_test_id']]['cpt_code_id']]['cpt_code']}} @endif</p>
                                                                    </td>
                                                                    @endif
                                                                </tr>
                                                            </table>
