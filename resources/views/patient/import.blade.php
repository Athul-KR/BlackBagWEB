<div class="text-center mb-3 px-lg-5 px-3">
                                      <h4 class="text-center fw-medium mb-0 ">Import Patient Data</h4>
                                    ...
                                    <div class="files-container justify-content-center mb-3">
                                      <div class="fileBody">
                                        ...
                                        <a href="{{ url('/patients/download')}}" class="dwld-btn" ><span class="material-symbols-outlined me-1 primary">download</span></a>
                                      </div>
                                    </div>
                                    ...
                                    <!-- <label class="mb-2">Patient</label> -->
                                      ...
                                      <form action="{{ url('/importPatient') }}" class="dropzone mb-3" id="docupload" enctype="multipart/form-data">
...
$(document).ready(function () {
    var uplodURL = '{{ url("patients/importpatient") }}';
        ...
        init: function() {
            ...
            this.on("addedfile", function(file) {
                if (isUploading) {
                    ...
                    swal("Warning!", 'Please wait until the current file is uploaded.', "warning");
                ...
                if ($.inArray(ext, ['xls', 'xlsx', 'csv']) == -1) {
                    ...
                    swal("Warning!", 'Invalid file format .Only .xls, .xlsx, and .csv files are allowed', "warning");
            ...
            this.on('success', function(file, response) {
                ...
                $('#import_Patient').modal('hide');

                if (response.success) {
                  generatePreview(response.import_key);

                    // Create a preview of valid and error records
                    // generatePreview(response.validRecords, response.errorRecords,response.importDocIDs);
                } else {
            ...
            this.on('error', function(file, response) {
              console.error("Upload error:", response); // Log the full error response
              swal("Warning!", "An error occurred while uploading the file. " + response, "warning");
    ...
    function generatePreview(import_key) {
      window.location.href = "{{url('patients/excelpreview/')}}" +'/'+ import_key ;
